...
PLUGIN_NAME = "get_stock_close_price_to_csv"
PLUGIN_DESCRIPTION = "Writes stock close price data for ticker symbol to a csv file. It gathers data for a provided time window."
ARGS_SCHEMA = {
...
class GetStockClosePriceToCsv(Plugin):
    name = PLUGIN_NAME
    ...
    async def arun(ticker: str, filename: str = None, date_from: str = None, date_to: str = None) -> Execution:
        try:
            ...
            if not filename:
                filename = f"{ticker}_close_price.csv"
            else:
            ...
            dataframe.to_csv(filename, encoding='utf-8', columns=["Close"])
            return Execution(
                observation=f"Stock Close data was successfully written to {filename}."
            )
